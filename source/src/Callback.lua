...
if Text and Text:match('(%d+)/meza_photo') then
  ...
  --
--------by 7oda
if Text and Text:match('(%d+)/likeId_ph/(%d+)/(.*)') then
local Data = {Text:match('(%d+)/likeId_ph/(%d+)/(.*)')}
local UserId = Data[1]
local id_photo = Data[3]
if Redis:sismember(Fast.."like:users:"..UserId..id_photo, IdUser) then
return bot.answerCallbackQuery(data.id,"\n⦁ لقد قمت بالتصويت من قبل", true)
end
Redis:sadd(Fast.."like:users:"..UserId..id_photo, IdUser)
local num = tonumber(#Redis:smembers(Fast.."like:users:"..UserId..id_photo))
...
data = {
{
{text = num.." "..emo , data = UserId.."/likeId_ph/"..num..'/'..id_photo}, 
},
}
}
if Msg.caption then
